...
export default function AdCard ({ad,className}:AdCardProps){
  return <div className={cn(styles.adCard,{},[className])}>
    <div className={styles.imageWrapper}>
      {ad?.images && ad?.images[0] !== 'undefined'?<Image className={styles.image} fill src={`${process.env.IMAGE_SERVER_URL}${ad?.images[0]}`} alt={'Ad main photo'}/>:<div className={styles.noImage}>No Image</div>}
    </div>
